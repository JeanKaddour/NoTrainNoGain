...
def get_schedule_fn(cfg_train):
    ...
    """
    if "budget" in cfg_train and cfg_train.budget == "steps":
        if "budget" in cfg_train.scheduler:
            raise ValueError("Cannot use a budget scheduler with a step budget")
    ...
    # reference hardware time schedulers
    elif cfg_train.scheduler == "budget-one-cycle-seconds":
        scheduler_fn = partial(
            get_budget_one_cycle_seconds,
            budget_seconds=cfg_train.budget * 3600,
            num_training_steps=cfg_train.steps,
        )
    elif cfg_train.scheduler == "budget-triangle2-seconds":
        scheduler_fn = partial(
            get_budget_triangle_seconds,
            budget_seconds=cfg_train.budget * 3600,
            num_training_steps=cfg_train.steps,
            falloff=0.25,
            base_percentage=0.25,
...
def get_budget_triangle_seconds(optimizer, budget_seconds, num_training_steps, base_percentage=0.5, falloff=0.5):
    """Linear increase from a percentage of the base learning rate, then linear decay.

    plot min(0.5 + x * (1 - 0.5)/(1-0.25) / 1000, 1/0.25 - x / (1000 * 0.25)) from 0 to 1000 in the plot range 0 to 1
    """

    def lr_lambda(seconds_elapsed):
        fake_step = _get_fake_step_seconds(seconds_elapsed, budget_seconds, num_training_steps)
        return min(
            base_percentage + fake_step * (1 - base_percentage) / (1 - falloff) / num_training_steps,
            float(1 / falloff - fake_step / (num_training_steps * falloff)),
        )
...
def get_budget_one_cycle_seconds(optimizer, budget_seconds, num_training_steps):
    """Simple single-cycle scheduler using elapsed seconds. Not including paper/fastai three-phase things or asymmetry."""
