...
from efficient_training import stacking
...
from .utils import (
    group_parameters,
    prepare_pretraining_dataloader,
    torchdynamo_compile_method,
    update_ema,
    updated_latest_weight_average,
)
...
def initialize_torch(model, dataset, validation_set, tokenizer, cfg_train, cfg_impl, setup=_default_setup):
    """initialize a torch engine."""
    if dataset is not None:
        dataloader = prepare_pretraining_dataloader(dataset, tokenizer, cfg_train, cfg_impl)
        validation_dataloader = (
            prepare_pretraining_dataloader(validation_set, tokenizer, cfg_train, cfg_impl, is_validation=True)
    ...
    model_engine.train()
    return model_engine, model_engine.optimizer, model_engine.scheduler, dataloader, validation_dataloader
...
class TorchEngine(torch.nn.Module):
    ...
    def __init__(self, model, cfg_train, cfg_impl, setup=_default_setup, seq_length=128):
        ...
        self.update_steps = 0  # Record the number of update steps
        ...
        if "stacking" in cfg_train:
            self.stacking_scheduler = stacking.get_stacking_scheduler(
                model=self.model,
                optimizer=self.optimizer,
                num_active_layers=cfg_train.stacking.num_initial_layers,
                num_layers_to_add=cfg_train.stacking.num_layers_to_add,
                start_steps=0,
                T_max=int(cfg_train.stacking.T_max_factor * cfg_train.steps),
                num_train_steps=cfg_train.steps,
                enabled=cfg_train.stacking.enabled,
                batch_size=cfg_train.batch_size,
                train_dataset=None,
                stacking_scheduler_type=cfg_train.stacking.scheduler,
                copy_optim_states=cfg_train.stacking.copy_optim_states,
                manual_balance_factor=cfg_train.stacking.manual_scheduler.balance_factor,
                manual_function=cfg_train.stacking.manual_scheduler.function,
                freeze_bottom_layers=cfg_train.stacking.freeze_bottom_layers,
                adjust_lr=cfg_train.stacking.adjust_lr,
                warmup_steps=cfg_train.warmup_steps,
                doubling=cfg_train.stacking.doubling,
                doubling_interpolation=cfg_train.stacking.doubling_interpolation,
                step_fractions=cfg_train.stacking.step_fractions,
                seconds_budget=None if cfg_train.budget == "steps" else cfg_train.budget * 3600,
            )
        else:
            self.stacking_scheduler = None
    ...
    def get_num_active_layers(self) -> int:
        return self.model.get_num_active_layers()

    def step(self, batch: dict[str, torch.Tensor], seconds_counter: Optional[float] = None) -> torch.Tensor:
        loss = self.forward(**batch)["loss"]
        self.backward(loss)

        self.optimizer_step(seconds_counter)
    ...
    @torch.no_grad()
    def forward_losses(self, batch):
        with torch.autocast(**self.amp_settings):
            return self.model.forward_all_losses(**batch)

    def optimizer_step(self, seconds_counter: Optional[float] = None):
        ...
        if self.accumulated_samples >= self.current_batch_size:
            ...
            self.schedule_batch_size(seconds_counter)
            self.schedule_curriculum(seconds_counter)
            self.moving_average_computation()
            self.update_steps += 1
        self.scheduler.step(seconds_counter)  # Trigger in every step, otherwise things get annoying with grad accumulation
        if self.stacking_scheduler is not None:
            self.stacking_scheduler.step(seconds_counter)
    ...
    def schedule_batch_size(self, step: Optional[float] = None):
        """Optionally implement linear batch size ramp-ups."""
        if (self.cfg_train.batch_size_ramp > 0) and (self.cfg_train.batch_size_ramp < 1):
            if self.cfg_train.budget == "steps":
                raise NotImplementedError("Can't schedule batch size if budget is in steps")

            # interpret as percentage of total budget
            elapsed_seconds = step
            fake_step = elapsed_seconds / (3600 * self.cfg_train.budget) * self.cfg_train.steps
    ...
    def schedule_curriculum(self, step: Optional[float] = None):
        """Optionally implement linear sequence lengths curriculum."""
        if self.sequence_curriculum:
            if self.cfg_train.budget == "steps":
                raise NotImplementedError("Cannot use sequence curriculum with step budget")
            ...
            fraction_elapsed = step / (3600 * self.cfg_train.budget)
    ...
    def save_training_checkpoint(self, identifier, directory="checkpoints", state=None):
        ...
        file = os.path.join(directory, f"{identifier_str}.pth")
...
def _load_optimizer(model, cfg_train, cfg_impl):
    ...
    elif cfg_train.optim.type == "SophiaG":
