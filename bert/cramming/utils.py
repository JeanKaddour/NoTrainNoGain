...
def find_pretrained_checkpoint(cfg, downstream_classes=None):
    ...
    if checkpoint_name is not None:
        ...
        if cfg.eval.arch_modifications is not None:
            log.info(f"Modifying architecture with {cfg.eval.arch_modifications}")
            cfg_arch = OmegaConf.merge(cfg_arch, cfg.eval.arch_modifications)
        model_file = os.path.join(checkpoint_name, "model.pth")

        print(cfg_arch)

    if cfg.eval.model_pth is not None:
        model_file = cfg.eval.model_pth
...
def _initialize_wandb(setup, cfg):
    if is_main_process():
        ...
        if "resume" in cfg.wandb and cfg.wandb.resume is not None:
            resume = "must"
            resume_id = cfg.wandb.resume
        else:
            resume = None
            resume_id = None

        if "wandb_name" in cfg and cfg.wandb_name is not None:
            name = cfg.wandb_name
        else:
            name = cfg.name
...
@torch.no_grad()
def validate(model_engine, validloader, device):
    ...
    return loss.item() / len(validloader)
