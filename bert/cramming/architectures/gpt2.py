...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def set_active_layers(self, num_active_layers: int) -> None:
        ...
        self.transformer.active_layer_indices = indices

    def get_num_active_layers(self) -> int:
        return (
            len(self.transformer.active_layer_indices) if len(self.transformer.active_layer_indices) > 0 else self.config.num_hidden_layers
        )
    ...
    def _reorder_cache(past_key_values: Tuple[Tuple[torch.Tensor]], beam_idx: torch.Tensor) -> Tuple[Tuple[torch.Tensor]]:
        ...
        """
        return tuple(
            tuple(past_state.index_select(0, beam_idx.to(past_state.device)) for past_state in layer_past) for layer_past in past_key_values
        )
