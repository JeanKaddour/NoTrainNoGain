...
class T5Stack(nn.Module, ModuleUtilsMixin):
    ...
    def add_t5_block(self, i):
        if self.config.share_positional_bias:
            ...
            self.block.append(T5Block(self.config, has_relative_attention_bias=True))
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.block):
            if (
                len(self.active_layer_indices) > 0
                and i not in self.active_layer_indices
            ):
                continue
...
class MyT5(nn.Module):
    ...
    def get_num_active_layers(self) -> int:
        # technically, it would be len(self.encoder.active_layer_indices) + len(self.decoder.active_layer_indices)
        # but this makes things a bit more complicated with the way stacking/dropping works right now
